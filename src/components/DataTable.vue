<template>
    ...
    <v-card flat>
        <v-card-title class="d-flex align-center pe-2 mb-3">
            <v-icon icon="mdi-book"></v-icon> &nbsp;
            My Liked Books
        ...
        <v-data-table :headers="headers" :items="data" :search="search" item-value="name">
            <template v-slot:top>
                <v-text-field v-model="search" label="Search" class="pa-2"></v-text-field>
            </template>
        </v-data-table>
    </v-card>
</template>
